...
constexpr T accumulate(Iterable&& i, T init, Fn&& fn) {
...
template <typename Iterable, typename T>
[[nodiscard]] constexpr T accumulate(Iterable&& i, T init) {
  return detail::accumulate(std::forward<Iterable>(i), std::forward<T>(init));
}

template <typename Iterable, typename T, typename Fn>
[[nodiscard]] constexpr T accumulate(Iterable&& i, T init, Fn&& fn) {
  return detail::accumulate(std::forward<Iterable>(i), std::forward<T>(init),
                            std::forward<Fn>(fn));
}
