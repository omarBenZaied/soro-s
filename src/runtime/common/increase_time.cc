...
interval get_interval(si::length distance) {
  ...
  return it->distance_ == distance && it->distance_ < intr_points.back().distance_ ? interval{&*it, &*(it + 1)}
                                   : interval{&*(it - 1), &*it};
}
...
train_state find_state_with_speed(si::speed const& speed,
                                  ...
                                  bool const& accel,vector<train_state>::const_iterator& start) {
  utls::sassert(start>=states.begin()&&start<=states.end(),"start isnt a valid iterator");
  auto it = std::find_if(start,states.end(), [speed, accel](train_state const& state) {
    return accel ? state.speed_ >= speed : state.speed_ <= speed;
  });
  utls::sassert(it != states.end(), "no state with speed {} found", speed);
  start = it->speed_==speed?it:it-1;
...
train_state find_state_with_speed(si::speed const& speed,
                                  vector<train_state> const& states,
                                  bool const& accel) {
  auto start_it = states.begin();
  return find_state_with_speed(speed,states,accel,start_it);
}
...
bool cruise_speed_sufficient(train_state const& start_state,si::length const& end_distance, si::time const& desired_time) {
  return start_state.time_+get_cruise_time(start_state.speed_,start_state.dist_,end_distance)>=desired_time;
}
bool cruise_speed_sufficient(train_state const& start_state,train_state const& end_state,si::time const& planned_dif) {
  auto t_old = end_state.time_-start_state.time_;
  return get_cruise_time(start_state,end_state)-t_old>=planned_dif;
}

vector<train_state> search_for_cruise(vector<train_state> accel_phase,si::speed const& speed,vector<train_state> brake_phase,si::time const& planned_dif) {
  auto brake_it = brake_phase.begin();
  for(int i=accel_phase.size()-2; i>=0&&accel_phase[i].speed_>=speed; --i) {
    auto end_cruise = find_state_with_speed(accel_phase[i].speed_,brake_phase,false,brake_it);
    if(cruise_speed_sufficient(accel_phase[i],end_cruise,planned_dif)) {
      return {accel_phase[i],end_cruise};
    }
  }
  auto start_cruise = find_state_with_speed(speed,accel_phase,true);
  auto end_cruise = find_state_with_speed(speed,brake_phase,false,brake_it);
...
bool check_AHD(train_drive& drive, int const& offset) {
  ...
  if (!cruise_speed_sufficient(end_accel,end_new_cruise,planned_dif)) {
    make_result_AHD(drive, offset, accel_phase, brake_phase, min_cruise_v,
                    end_new_cruise, end_accel);
    return false;
  }
  auto cruise_phase = search_for_cruise(accel_phase,min_cruise_v,brake_phase,planned_dif);
...
bool check_AHA(train_drive& drive, rs::train_physics const& tp,
  ...
  while (state.speed_ != lowest_speed) {
    ...
    utls::sassert(delta.speed_.is_positive(),"Non positive delta speed");
    ...
    if (cruise_speed_sufficient(accel_state,state,planned_dif)) {
      result_found = true;
      break;
    }
  }
...
si::speed find_optimal_speed(si::speed const& start_speed,si::accel const& accel,si::length const& start_dist,si::length const& end_dist,si::time const& time) {
...
vector<train_state> continue_brake(train_state& end_brake,rs::train_physics const& tp,
  ...
  while(end_brake.speed_>min_speed&&end_brake.dist_<max_dist) {
    ...
    if(!end_brake.speed_.is_zero()&&end_cruise_time==target_time) {
      return result;
    }
    if(end_brake.speed_.is_zero()||end_cruise_time>target_time) {
      result.pop_back();
...
bool check_HDH(train_drive& drive, rs::train_physics const& tp,
  ...
  auto planned_dif = pt.e_time_-drive.phases_.back().back().time_;
  //The total time this section should take
  ...
  drive.erase_elements(offset, 2);
  auto new_brake = continue_brake(cruise_phase.front(),tp,v_opt,si::time::infinity(),brake_phase.back().dist_);
  auto end_cruise = v_opt==brake_phase.back().speed_?brake_phase.back():find_state_with_speed(v_opt,brake_phase,false);
  std::erase_if(brake_phase, [v_opt](train_state const& state) {return state.speed_>=v_opt;});
  //end_cruise has the wrong time. The time however gets fixed by fix_drive.
  brake_phase.insert(brake_phase.begin(),end_cruise);
...
bool check_DHA(train_drive& drive, rs::train_physics const& tp,
  ...
  throw utl::fail("DHA not implemented yet");
}
...
train_state find_end_of_new_acceleration(
  ...
  for (i = accel_phase.size() - 2; i >= 0 && accel_phase[i].speed_ >= pt.v_min_;
       --i) {
    if(cruise_speed_sufficient(accel_phase[i],pt.distance_,pt.e_time_)) return accel_phase[i];
  }
  if(i==-1) return accel_phase.front();
  auto it = accel_phase.begin()+i;
  return find_state_with_speed(pt.v_min_, accel_phase, true,it);
}
...
void make_result_HA_DA(train_state const& end_accel, train_drive& drive) {
  ...
  if(drive.phases_.back().size()<2) {
    drive.phases_.pop_back();
    drive.phase_types_.pop_back();
  }
  drive.push_back(new_cruise,cruising);
  drive.fix_phases(0);
}
...
bool check_HA(train_drive& drive, rs::train_physics const& tp) {
  ...
  auto end_accel = find_end_of_new_acceleration(accel_phase);
...
bool check_DH(train_drive& drive,rs::train_physics const& tp){
  ...
  auto float_precision = si::time(FP_PRECISION<si::time::precision>);
  if(end_time<pt.e_time_&&(pt.e_time_-end_time)>=float_precision) check_DHA(drive,tp,0);
  return true;
}
...
bool check_A(train_drive& drive,rs::train_physics const& tp) {
  ...
  train_state end_accel = find_end_of_new_acceleration(phase);
  utls::sassert(end_accel.speed_.is_positive(),"check_A created zero speed cruise phase");
  auto it = utls::find_if(phase,[end_accel](train_state const& state){return state.speed_>=end_accel.speed_;});
  phase.erase(it,phase.end());
  phase.push_back(end_accel);
...
bool check_H(train_drive& drive,rs::train_physics const& tp) {
  ...
  if(brake_state.dist_==pt.distance_&&pt.e_time_-brake_state.time_>float_precision) {
    throw std::exception("even slowest drive fails");
  }
  ...
  if((pt.e_time_-end_time)>float_precision) check_DHA(drive,tp,0);
  return true;
}
